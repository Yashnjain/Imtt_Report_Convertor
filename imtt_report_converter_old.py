...
def get_email_date(browser, x_path_i):
    try:
        ...
        try:
            download_xpath = f'/html/body/div[{x_path_i}]/div/div[2]/div[2]/div/div/div/div[3]/div[2]/div/div[1]/div[2]/div/div/div/div/div/div[6]/div/div' #change 7 to6  for normal run
            WebDriverWait(browser, 90, poll_frequency=1).until(EC.element_to_be_clickable((By.XPATH,download_xpath))).click()
        except:
            try:
                download_xpath = f'/html/body/div[{x_path_i}]/div/div[2]/div[2]/div[2]/div/div/div/div[2]/div/div[1]/div[2]/div/div/div/div/div/div[6]/div/div'
...
def main():
    ...
    while retry < 2:
        ...
        try:
            ...
            x_path_i = 2
            #getting the current email date to set download folder
            email_date, email_date2, x_path_i = get_email_date(browser, x_path_i)
            ...
            if date_today != email_date:
                ...
                if date_today != email_date:
                    ...
                    logging.info('send success e-mail for file already downloaded')
                    # log_json='[{"JOB_ID": "'+str(job_id)+'","CURRENT_DATETIME": "'+str(datetime.now())+'"}]'
                    
                    bu_alerts.send_mail(receiver_email = receiver_email,mail_subject ='JOB SUCCESS - {} No file found and file for today already downloaded'.format(job_name),mail_body = '{} completed successfully, Attached logs'.format(job_name),attachment_location = logfile)
